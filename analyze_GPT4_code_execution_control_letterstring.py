...
# Load data
data_fname = './' + args.gpt4_engine + '_code_execution_control_int' + str(args.interval_size)
...
for p in range(N_prob_types):
	...
	for t in range(N_trials_per_prob_type):
		...
		for i in range(len(response_parsed)):
			response_parsed[i] = response_parsed[i].replace('\n','')
		print(response_parsed)	# Print all responses (to ensure there are no parsing errors)
...
results_dir = './' + args.gpt4_engine + '_code_execution_control_int' + str(args.interval_size)
results_dir += '/'
